...
std::string NaniCmd::ParseCommandScriptLine(const ParseContext &ctx)
{
    ...
    {
        if (ref["rid"] == rid)
            return std::format("{}{}", !ctx.is_inline ? "@" : "", ParseCmd(ParseContext{ref, ctx.textMap, ctx.idx, ctx.cmds}));
    }
    ...
    for (const auto &cmdRef : inlined)
    {
        ...
        for (const auto &ref : ctx.refId)
        {
            ...
            if (ref["type"]["class"].get<std::string>() == "PrintText")
            {
                ...
                body += std::format("[{}]", ParseCmd(ParseContext{ref, ctx.textMap, ctx.idx, ctx.cmds, true}));
            break;
        }
    }
